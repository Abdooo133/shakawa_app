    ...
    if (!empty($firebase_uid) || !empty($email)) {
        ...
        if ($checkUser && $checkUser->num_rows > 0) {
            ...
            if ($conn->query($update_sql)) {
                $result = $conn->query("SELECT id FROM customers WHERE firebase_uid = '$firebase_uid' OR email = '$email' LIMIT 1");
                $existing = $result->fetch_assoc();
                echo json_encode(["status" => "success", "message" => "updated", "customer_id" => $existing['id']], JSON_UNESCAPED_UNICODE);
            } else {
            ...
            if ($conn->query($insert_sql)) {
                ...
                echo json_encode(["status" => "success", "message" => "inserted","customer_id" => $new_id], JSON_UNESCAPED_UNICODE);
            } else {
